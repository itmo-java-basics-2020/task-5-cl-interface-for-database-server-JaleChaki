...
import ru.andrey.kvstorage.commandresults.DatabaseUpdateKeyResult;
...
public class UpdateKeyCommand implements DatabaseCommand {
    ...
    public DatabaseCommandResult execute() {
        ...
        if (odb.isEmpty()) {
            return DatabaseUpdateKeyResult.CreateFailedResult(DatabaseUpdateKeyResult.ERROR_NOT_SUCH_DATABASE);
        }
        try {
            odb.get().write(tableName, keyName, valueName);
            return DatabaseUpdateKeyResult.CreateSuccessResult();
        }
        catch (DatabaseException e) {
            return DatabaseUpdateKeyResult.CreateFailedResult(e.getMessage());
        }
    }
