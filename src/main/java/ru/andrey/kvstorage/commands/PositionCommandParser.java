...
public class PositionCommandParser implements CommandParser {
    ...
    public DatabaseCommand Parse(ExecutionEnvironment context, String command) {
        if (Objects.isNull(command)) {
            return DatabaseUnsupportedCommand.INSTANCE;
        }
        String[] arguments = InternalParse(command);
        if (command.startsWith(CREATE_TABLE_COMMAND) && arguments.length == 2) {
            return new CreateTableCommand(context, arguments[0], arguments[1]);
        }
        if (command.startsWith(UPDATE_KEY_COMMAND) && arguments.length == 4) {
            return new UpdateKeyCommand(context, arguments[0], arguments[1], arguments[2], arguments[3]);
        }
        if (command.startsWith(READ_KEY_COMMAND) && arguments.length == 3) {
            return new DatabaseReadKeyCommand(context, arguments[0], arguments[1], arguments[2]);
        }
        return DatabaseUnsupportedCommand.INSTANCE;
    }
